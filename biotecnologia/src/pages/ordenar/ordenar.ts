...
export class OrdenarPage {
  ordenar:Observable<any[]>;
  pregunta:Observable<any[]>;
  constructor(public navCtrl: NavController, public navParams: NavParams,
    db: AngularFireDatabase, private alertCtrl: AlertController) {
    var id:number;
    id = navParams.get('param1');
    this.pregunta = db.list('/ordenar/'+id+'/pregunta').valueChanges();
    this.ordenar = db.list('/ordenar/'+id+'/opciones').valueChanges();
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad OrdenarPage');
  }
}
